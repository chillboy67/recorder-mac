...
因此加了一层**语言混合回退**：当声学分离明显失衡、且某段文字不属于考生所用语言的
文字系统时，按语言归属分配角色（雅思考生一律答英文，说其他语言的即教官——中文、
日文、韩文、俄文、泰文皆同）——这一步才让教官/考生分离真正可用。
已知缺口：法/德/西这类拉丁文教官与考生共用同一文字系统，需逐段语种检测才能区分。
...
不变。第 1、2 步已完成：设置接入引擎、命令行（`transcribe.py -l ja`）与界面
「语言」选择器（默认自动检测，共 15 种常用语言）；教官角色判定不再写死中文，
日／韩／俄／泰教官会话按文字系统归属分角色。已知缺口：法德西等拉丁文教官与考生
共用同一文字系统，需逐段语种检测才能区分。剩下第 3 步的分析与对照翻译。
