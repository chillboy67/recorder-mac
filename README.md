...
因此加了一层**语言混合回退**：当声学分离明显失衡且内容含中文时，按语言
归属分配角色（说中文的是教官，说英文的是考生）——这一步才让教官/考生分离
真正可用。
...
进度：语言工具层已落地为 `core/languages.py`，按 Unicode 文字系统判定语言
...
不变。第 1 步已完成：设置接入引擎与命令行（`transcribe.py -l ja`），
界面「语言」选择器同批落地（默认自动检测，共 15 种常用语言）；
第 2 步待做。
